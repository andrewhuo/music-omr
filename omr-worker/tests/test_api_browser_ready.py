...
class BrowserReadyApiTests(unittest.TestCase):
    def setUp(self):
        ...
        os.environ["BEDROCK_GENERAL_MODEL_ID"] = "global.anthropic.claude-sonnet-4-5-20250929-v1:0"
    ...
    def test_create_job_reuses_same_pdf_retry_request(self):
                            ...
                            "label_boxes": [
                                {
                                    ...
                                    "page": 1,
                                    "text": "1",
                                    "rect": {"left": 10, "top": 5, "right": 20, "bottom": 18},
                                    "hidden": False,
                                }
                            ],
                        }
        ...
        self.assertEqual(len((body.get("editable_state") or {}).get("label_boxes") or []), 1)
    ...
    def test_normalize_ai_suggestions_result_treats_bad_multirest_as_normal(self):
        editable_state = (self._sample_mapping_summary().get("editable_state") or {})
        raw_result = {
            "provider": "claude",
            "suggestions": [
        ...
        by_measure_id = normalized.get("by_measure_id") or {}
        self.assertNotIn("p1_s0_m0", by_measure_id)
        ...
        self.assertEqual(((normalized.get("summary") or {}).get("normal_measures_omitted")), 2)
        warnings = normalized.get("warnings") or []
        self.assertTrue(any("treated multi_measure_rest as normal" in str((row or {}).get("message") or "").lower() for row in warnings))
        self.assertGreaterEqual(
            len([row for row in warnings if (row or {}).get("type") == "normalization_adjusted"]),
            2,
        )

    def test_normalize_ai_suggestions_result_ignores_ending_suggestions(self):
        editable_state = {
        ...
        by_measure_id = normalized.get("by_measure_id") or {}
        self.assertNotIn("m0", by_measure_id)
        self.assertNotIn("m1", by_measure_id)
        ...
        warnings = normalized.get("warnings") or []
        self.assertEqual(
            len([row for row in warnings if "Ignored unsupported AI ending label" in ((row or {}).get("message") or "")]),
            2,
        )
        self.assertTrue(any("Dropped invalid maybe_label" in ((row or {}).get("message") or "") for row in warnings))

    def test_normalize_ai_suggestions_result_ignores_retired_false_measure(self):
        editable_state = {
            "systems": [{"system_id": "p1_s0", "page": 1, "system_index": 0}],
            "measures": [
        ...
        warnings = normalized.get("warnings") or []
        self.assertTrue(
            any("Ignored retired false_measure label" in ((row or {}).get("message") or "") for row in warnings)
        )
...
```""",
    ...
    def test_build_system_measure_request_single_prompt_excludes_multi_staff_rules(self):
        ...
        self.assertIn("Cut time looks like a large C with a vertical slash through it and means 2/2.", rules_text)
        ...
        self.assertIn("Count this one staff's written duration only.", rules_text)
        ...
        self.assertIn("For single-staff multi-measure rests, inspect the one visible staff.", rules_text)
        ...
        self.assertIn("Later measures must be normal unless they are a valid multi_measure_rest", rules_text)
    ...
    def test_build_system_measure_request_grand_prompt_includes_treble_bass_rules(self):
        ...
        rules = ((intro.get("instructions") or {}).get("rules")) or []
        rules_text = "\n".join(str(row) for row in rules)
        self.assertIn("For all AI Suggest decisions in grand-staff/piano music, use only the top staff/treble staff.", rules_text)
        self.assertIn("Ignore the bottom staff completely for time signature, pickup, and multi-measure rest.", rules_text)
        self.assertIn("do not switch to the bottom staff; make the best top-staff choice with low confidence.", rules_text)
    ...
    def test_build_system_measure_request_score_prompt_includes_score_rules_without_multi_rest_jump(self):
        ...
        self.assertIn("Staff means one set of 5 horizontal music lines", rules_text)
        self.assertIn("System means the full horizontal row of music containing all staves", rules_text)
        self.assertIn("For score pickup, start at the top visible staff.", rules_text)
        self.assertIn("If a staff shows only a full-measure rest in the first measure, skip to the next staff down.", rules_text)
        self.assertIn("Use the first staff with notes or readable rhythm; do not add multiple staves.", rules_text)
        self.assertIn("If every staff is resting or unreadable, make the best pickup-or-normal choice with low confidence.", rules_text)
        self.assertIn("Use the first active staff's visible meter in this crop.", rules_text)
        self.assertIn("If the first active staff meter is unreadable but common/cut/numeric meter is partially visible", rules_text)
        self.assertIn("Common time looks like a large C after the clef/key signature and means 4/4.", rules_text)
        self.assertIn("Cut time looks like a large C with a vertical slash through it and means 2/2.", rules_text)
        self.assertIn("Basic note values: filled notehead with stem = quarter note", rules_text)
        self.assertIn("identify the first active staff's notehead fill before deciding note value", rules_text)
        self.assertIn("A dot immediately to the right of a note/rest adds half its value.", rules_text)
        self.assertIn("A triplet is marked by a small 3 above or below a group", rules_text)
        self.assertIn("For the first measure, arithmetic wins over context", rules_text)
        self.assertIn("Always make the best pickup-or-normal choice", rules_text)
        self.assertIn("For non-first measures, do not judge pickup or beat completeness.", rules_text)
        self.assertIn("Later measures must be normal unless they are a valid multi_measure_rest", rules_text)
        self.assertIn("Ending / volta detection:", rules_text)
        ...
        self.assertNotIn("Do not inspect, compare, add, or use lower staves as fallback.", rules_text)
        self.assertNotIn("For the first measure, if the first active staff's written duration looks clearly shorter", rules_text)
        self.assertNotIn("For full score V1, do not use multi_measure_rest jumps.", rules_text)
    ...
    def test_build_system_measure_request_missing_score_type_uses_legacy_prompt(self):
        ...
        self.assertIn("Do not remember, inherit, carry, or track time signatures across measures.", rules_text)
        self.assertIn("Set measure_completeness only as needed", rules_text)
        self.assertIn("Always make the best useful choice from the allowed labels", rules_text)
        self.assertIn("If measure_completeness is unclear, you may include unclear_reason", rules_text)
        ...
        self.assertIn("If the first measure is hard to read, make the best pickup-or-normal choice", rules_text)
        self.assertIn("never return uncertain or maybe fields", rules_text)
        self.assertIn("Ending / volta detection:", rules_text)
    ...
    def test_render_corrected_pdf_applies_label_erase_before_labels(self):
        ...
        label_rect = page.draw_rect_calls[1]["rect"]
        self.assertEqual(
            editable_state.get("label_boxes"),
            [
                {
                    "label_id": "label:p1_s0_m0",
                    "measure_id": "p1_s0_m0",
    ...
    def test_render_corrected_pdf_label_boxes_follow_system_only_mode(self):
        page = _FakePage(_FakeRect(0, 0, 200, 160))
        fake_doc = _FakeDoc([page])
        systems = [
            {"system_id": "p1_s0", "page": 1, "system_index": 0, "current_value": "1"},
            {"system_id": "p1_s1", "page": 1, "system_index": 1, "current_value": "3"},
        ]
        measures = [
            {"measure_id": "p1_s0_m0", "system_id": "p1_s0", "page": 1, "system_index": 0, "measure_local_index": 0, "x_left": 20, "y_top": 30, "y_bottom": 60},
            {"measure_id": "p1_s0_m1", "system_id": "p1_s0", "page": 1, "system_index": 0, "measure_local_index": 1, "x_left": 60, "y_top": 30, "y_bottom": 60},
            {"measure_id": "p1_s1_m0", "system_id": "p1_s1", "page": 1, "system_index": 1, "measure_local_index": 0, "x_left": 20, "y_top": 90, "y_bottom": 120},
        ]
        editable_state = {"labels_mode": "system_only", "systems": systems, "measures": measures}
        ...
        self.assertEqual([box["measure_id"] for box in editable_state.get("label_boxes") or []], ["p1_s0_m0", "p1_s1_m0"])
        self.assertEqual([box["text"] for box in editable_state.get("label_boxes") or []], ["1", "3"])

    def test_render_corrected_pdf_label_boxes_follow_all_measures_mode(self):
        page = _FakePage(_FakeRect(0, 0, 200, 160))
        fake_doc = _FakeDoc([page])
        systems = [{"system_id": "p1_s0", "page": 1, "system_index": 0, "current_value": "1"}]
        measures = [
        ...
        editable_state = {"labels_mode": "all_measures", "systems": systems, "measures": measures}
    ...
    def test_render_corrected_pdf_draws_moved_label_and_reports_moved_box(self):
        page = _FakePage(_FakeRect(0, 0, 200, 160))
        fake_doc = _FakeDoc([page])
        systems = [{"system_id": "p1_s0", "page": 1, "system_index": 0, "current_value": "1"}]
        measures = [
        ...
        editable_state = {
            ...
            "systems": systems,
            "measures": measures,
            "label_positions": {"label:p1_s0_m0": {"page": 1, "left": 44.0, "top": 18.0}},
        }
        ...
        self.assertEqual(drawn, 1)
        self.assertEqual(page.insert_text_calls[0]["point"], (45.0, 29.0))
        self.assertEqual(
            editable_state.get("label_boxes"),
            [
                {
                    ...
                    "page": 1,
                    "text": "1",
                    "rect": {"left": 44.0, "top": 18.0, "right": 52.0, "bottom": 30.0},
                    "hidden": False,
                }
            ],
        )

    def test_render_corrected_pdf_hidden_label_wins_over_moved_label(self):
        page = _FakePage(_FakeRect(0, 0, 200, 160))
        fake_doc = _FakeDoc([page])
        systems = [{"system_id": "p1_s0", "page": 1, "system_index": 0, "current_value": "1"}]
        measures = [
        ...
        self.assertEqual(page.insert_text_calls, [])
        self.assertTrue(editable_state["label_boxes"][0]["hidden"])
        self.assertEqual(editable_state["label_boxes"][0]["rect"], {"left": 44.0, "top": 18.0, "right": 52.0, "bottom": 30.0})
    ...
    def test_build_system_measure_request_includes_unified_multi_rest_guidance(self):
        mapping_summary = self._sample_mapping_summary()
        editable_state = mapping_summary.get("editable_state") or {}
        system_row = (editable_state.get("systems") or [])[0]
        measure_rows = [row for row in (editable_state.get("measures") or []) if row.get("system_id") == "p1_s0"]
    ...
    def test_build_system_measure_request_includes_reference_examples_before_real_measures(self):
        ...
        with TemporaryDirectory() as tmpdir:
            ...
            (tmp_path / "old_style_rest_positive_16.png").write_bytes(b"large-count-reference")
            with (
