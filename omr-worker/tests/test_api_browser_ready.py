...
```""",
    ...
    def test_build_system_measure_request_score_prompt_includes_score_rules_without_multi_rest_jump(self):
        ...
        self.assertIn("For pickup detection in full scores, use only the top visible staff.", rules_text)
        self.assertIn("Ignore all lower staves completely for pickup duration.", rules_text)
        self.assertIn("Do not inspect, compare, add, or use lower staves as fallback.", rules_text)
        self.assertIn("Use only the top staff's visible meter in this crop.", rules_text)
        self.assertIn("make the best reasonable meter read and continue", rules_text)
        self.assertIn("Common time looks like a large C after the clef/key signature and means 4/4.", rules_text)
        self.assertIn("Cut time looks like a large C with a vertical slash through it and means 2/2.", rules_text)
        self.assertIn("Basic note values: filled notehead with stem = quarter note", rules_text)
        self.assertIn("identify the top-staff notehead fill before deciding note value", rules_text)
